...
export function Sidebar() {
  ...
  const getCategoryActiveClass = (color: string) => {
    switch (color) {
      ...
      case "indigo": return "bg-gradient-to-r from-indigo-500/20 to-indigo-600/20"; // for "All"
  ...
  const getCategoryHoverClass = (color: string) => {
    switch (color) {
      ...
      case "indigo": return "hover:bg-gradient-to-r hover:from-indigo-500/10 hover:to-indigo-600/10";
  ...
  const getCategoryBulletClass = (color: string) => {
    switch (color) {
      ...
      case "indigo": return "from-indigo-500 to-indigo-600"; // for "All"
      default: return "from-gray-500 to-gray-600";
    }
  };

  // Task filtering logic (same)
  const filterCounts = useMemo(() => {
    ...
    return { all, pending, completed, overdue, today, this_week: thisWeek } as Record<string, number>;
  }, [tasks]);
  ...
  const categories = [
    { key: null, label: "All", color: "indigo" }, // ✅ New "All" category
        ...
        <div className="glassmorphism bg-white/70 dark:bg-slate-800/70 rounded-2xl p-6 shadow-xl">
          ...
          <div className="space-y-2">
            {categories.map((category) => (
              <div key={category.key ?? "all"} className="flex items-center justify-between">
                <Button
                  variant="ghost"
                  onClick={() => setSelectedCategory(category.key)}
                  className={cn(
                    "flex items-center space-x-3 px-3 py-2 rounded-lg transition-all duration-300",
                    (selectedCategory === category.key || (category.key === null && selectedCategory === null)) &&
                      getCategoryActiveClass(category.color),
                    selectedCategory !== category.key && getCategoryHoverClass(category.color)
                  )}
                >
                  <div
                    className={cn("w-3 h-3 rounded-full bg-gradient-to-r", getCategoryBulletClass(category.color))}
                  />
